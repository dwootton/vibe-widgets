...
const GalleryItem = ({ example, index, mode, model }: { example: typeof EXAMPLES[0], index: number, mode: 'horizontal' | 'grid', model?: any, key?: React.Key }) => {
  ...
  return (
    <motion.div
      ...
      onClick={handleClick}
      className={`
                bg-white border-2 border-slate rounded-xl p-6 shadow-hard flex flex-col gap-4 group cursor-pointer
        ...
        <div className="sticky top-0 h-screen flex items-center overflow-hidden">
          <motion.div style={{ x: springX }} className="flex gap-12 px-12 md:px-24">
            {featuredExamples.map((ex, i) => (
              <GalleryItem key={ex.id} example={ex} index={i} mode="horizontal" model={getModelForExample(ex)} />
            ))}
