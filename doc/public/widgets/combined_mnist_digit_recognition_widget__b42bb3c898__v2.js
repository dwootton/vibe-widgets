...
export const ProgressBar = ({ label, value, isMax, html }) => {
  ...
  return html`
    <div style=${{ marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '10px' }}>
      ...
      <span style=${{ width: '40px', fontSize: '11px', textAlign: 'right', color: isMax ? '#4f46e5' : '#64748b' }}>
        ${(value * 100).toFixed(0)}%
      </span>
    </div>
  `;
...
export default function DigitRecognizerWidget({ model, html, React }) {
    ...
    <div style=${{ 
        ...
        <div style=${{ display: 'flex', gap: '12px' }}>
          ...
          <button 
            ...
            style=${{ 
              ...
              background: '#4f46e5', color: 'white', cursor: 'pointer', fontWeight: 600
            }}
          >Recognize</button>
        </div>
      </div>
      ...
      <div style=${{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style=${{ 
          ...
          <div style=${{ fontSize: '14px', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            Result
          </div>
