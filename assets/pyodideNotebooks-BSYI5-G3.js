const r=[{type:"markdown",content:`
    ...
    `},{type:"code",content:`import vibe_widget as vw
import pandas as pd

vw.models()`,defaultCollapsed:!0,label:"Setup"},{type:"code",content:`# Configure (demo mode - no actual LLM calls)
vw.config(
    model="google/gemini-3-flash-preview",
    api_key="demo-key"
)`,defaultCollapsed:!0,label:"Config"},{type:"markdown",content:`
    ...
    `},{type:"code",content:`# Create 3D Solar System widget
...
solar_system`,label:"3D Solar System"},{type:"markdown",content:`
    ...
    `},{type:"code",content:`# Create interactive Hacker News widget
hn_clone = vw.create(
...
hn_clone`,label:"Hacker News Clone"},{type:"markdown",content:`
      <h3>How It Works</h3>
...
hn_clone = vw.create(
    ...
    `,defaultCollapsed:!0}],i=[{type:"markdown",content:`
...
vw.models()`,defaultCollapsed:!0,label:"Setup"},{type:"code",content:`# Configure (demo mode)
vw.config(
    model="google/gemini-3-flash-preview",
    api_key="demo-key"
)`,defaultCollapsed:!0,label:"Config"},{type:"code",content:`# Load COVID-19 data
print(f"COVID-19 data loaded: {len(covid_df)} days")
print(f"Columns: {list(covid_df.columns)}")
covid_df.head(3)`,defaultCollapsed:!0,label:"Load Data"},{type:"markdown",content:`
      <h3>Step 1: Basic Line Chart</h3>
      <p>Create a simple line chart showing COVID-19 trends over time.</p>
    `},{type:"code",content:`# Create basic line chart
timeline = vw.create(
    "line chart showing confirmed, deaths, recovered over time",
    data=covid_df
)

timeline`,label:"Basic Chart"},{type:"markdown",content:`
      <h3>Step 2: Add Interactive Hover</h3>
      <p>Use <code>vw.edit()</code> to add a vertical dashed line when hovering.</p>
    `},{type:"code",content:`# Edit to add interactive hover crosshair
timeline_v2 = vw.edit(
...
timeline_v2`,label:"Enhanced Chart"},{type:"markdown",content:`
...
chart_v3 = vw.edit(
    ...
    `,defaultCollapsed:!0}],n=[{type:"markdown",content:`
...
vw.config(
    model="google/gemini-3-flash-preview",
    api_key="demo-key"
)`,defaultCollapsed:!0,label:"Config"},{type:"code",content:`# Load Seattle weather data
# (data is pre-loaded from /testdata/seattle-weather.csv)
print(f"Weather data loaded: {len(data)} rows")
print(f"Columns: {list(data.columns)}")
data.head(3)`,defaultCollapsed:!0,label:"Load Data"},{type:"markdown",content:`
    ...
    `,defaultCollapsed:!0},{type:"code",content:`# Create scatter plot that outputs selected indices
scatter = vw.create(
...
scatter`,label:"Scatter Plot"},{type:"markdown",content:`
    ...
    `,defaultCollapsed:!0},{type:"code",content:`# Create bar chart that inputs selected_indices
bars = vw.create(
...
bars`,label:"Bar Chart"},{type:"markdown",content:`
      <h3>How It Works</h3>
...
bars = vw.create(
    ...
    `,defaultCollapsed:!0}],s=[{type:"markdown",content:`
...
vw.config(model="google/gemini-3-flash-preview", api_key="demo")`,defaultCollapsed:!0,label:"Setup"},{type:"code",content:`# Check loaded training data
print(f"Loaded X_moves: {len(x_moves_df)} moves")
print(f"Loaded O_moves: {len(o_moves_df)} moves")
print(f"Columns: {list(x_moves_df.columns[:10])}...")`,defaultCollapsed:!0,label:"Data Check"},{type:"markdown",content:`
      <h3>Training the AI</h3>
      <p>We train two models - one for predicting rows and one for columns.</p>
    `,defaultCollapsed:!0},{type:"code",content:`# Feature columns (board state encoding)
...
`,defaultCollapsed:!0,label:"Train AI"},{type:"code",content:`# Helper functions for AI
...
def predict_best_move(board_state, player='o'):
    ...
    return best_move`,defaultCollapsed:!0,label:"AI Functions"},{type:"markdown",content:`
    ...
    `},{type:"code",content:`# Create the game board widget with proper outputs
...
game_board`,label:"Game Board"},{type:"code",content:`# Create AI controller widget that computes moves
import time
...
game_board`,label:"AI Controller"}],e=[{url:"/testdata/seattle-weather.csv",varName:"data"}],t=[{url:"/testdata/X_moves.csv",varName:"x_moves_df"},{url:"/testdata/O_moves.csv",varName:"o_moves_df"}],a=[{url:"/testdata/planets.csv",varName:"planets_df"},{url:"/testdata/hn_stories.json",varName:"hn_df",type:"json"}],o=[{url:"/testdata/day_wise.csv",varName:"covid_df"}],d={"cross-widget":e,tictactoe:t,"pdf-web":a,edit:o};export{n as CROSS_WIDGET_NOTEBOOK,d as NOTEBOOK_DATA_MAP,a as PDF_WEB_DATA_FILES,r as PDF_WEB_NOTEBOOK,o as REVISE_DATA_FILES,i as REVISE_NOTEBOOK,t as TICTACTOE_DATA_FILES,s as TICTACTOE_NOTEBOOK,e as WEATHER_DATA_FILES};
